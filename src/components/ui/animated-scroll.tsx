...
const pages = [
  {
    ...
    rightContent: {
      ...
      description:
        "Designing intuitive and visually engaging digital experiences that balance aesthetics with usability. From user research and wireframes to high-fidelity interfaces and interactive prototypes.",
      skills: [
        "User Research",
        "WireFraming",
        "Prototyping",
  ...
  },
  {
    ...
    leftContent: {
      ...
      description:
        "Building fast, scalable, and secure web applications tailored to business needs. From responsive websites to complex SaaS platforms with modern technologies.",
      skills: ["B2B", "B2C", "Web App", "SaaS", "Website"],
...
export default function ScrollAdventure() {
  ...
  const enterProgress = useSpring(enterProgressRaw, {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001,
  });
  ...
  const enterBorderRadius = useTransform(
    enterProgress,
    [0, 1],
    ["40px", "0px"],
  );
    ...
    >
      <motion.div
        style={{
          ...
          borderRadius: enterBorderRadius,
        }}
...
function PageSlide({
  ...
  return (
    <motion.div
      style={{ zIndex }}
      className="absolute inset-0 overflow-hidden bg-background dark:bg-black"
    >
      <motion.div
        style={{ y: leftY }}
            ...
            <div className="w-full h-full flex items-center justify-start p-12 md:p-24 lg:p-32 xl:p-40 relative group">
              <motion.div
                className={cn(
                  ...
                  page.leftContent?.hoverColor || "bg-primary/5",
                )}
                initial={{ height: 0 }}
              ...
              />
              {page.leftContent && (
                <EditorialContent content={page.leftContent} index={index} />
              )}
            </div>
          )}
        </div>
          ...
          {page.rightBgImage ? (
            ...
            <div className="w-full h-full flex items-center justify-start p-12 md:p-24 lg:p-32 xl:p-40 relative group">
              <motion.div
                className={cn(
                  "absolute inset-0 z-0",
                  page.rightContent?.hoverColor || "bg-primary/5",
                )}
                initial={{ height: 0 }}
                whileHover={{ height: "100%" }}
                transition={{ duration: 0.4 }}
              />
              {page.rightContent && (
                <EditorialContent content={page.rightContent} index={index} />
              )}
            </div>
          )}
        </div>
...
function BridgeSlide({
  ...
  const opacity = useTransform(
    scrollProgress,
    [base - step / 4, base + step / 4, 0.98, 1],
    [0, 1, 1, 0],
  );
  ...
  return (
    ...
    >
      <motion.div
        style={{ y }}
        className="space-y-16 max-w-[1200px] w-full px-[5%]"
      >
        <h2 className="text-4xl md:text-5xl lg:text-7xl font-medium tracking-tight text-foreground dark:text-white leading-[1.25] font-sans">
          Showcasing the digital products we've built{" "}
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn(
            "absolute inset-0 translate-y-[101%] group-hover/badge:translate-y-0 transition-transform duration-300 ease-out z-0",
            color.main,
          )}
        />
        <span
